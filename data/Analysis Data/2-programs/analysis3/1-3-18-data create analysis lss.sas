...
*Simple7V3;
data include2; *Simple7V3;
	set lss.Simple7V3(keep = subjid PA3cat);
	by subjid;
	run;
...
data include; *Create variables; 
  ...
  *Variable: BMI3cat;
  BMI3cat = .;
  if 30 <= BMI            & ^missing(BMI) then BMI3cat = 0; *Poor Health         (Obese: >= 30);
  if 25 <= BMI & BMI < 30 & ^missing(BMI) then BMI3cat = 1; *Intermediate Health (Overweight: 25-29.9);
  if             BMI < 25 & ^missing(BMI) then BMI3cat = 2; *Ideal Health        (Ideal: < 25);
  ...
  *Variable: nutrition3cat;
  nutrition3cat = .; *Not Available at Visit 3;
  ...
  *Variable: glucose3cat;
  if HbA1c3cat = 2 | FPG3cat = 2 | DMmeds = 1 then glucose3cat = 0;
  if HbA1c3cat = 1 | FPG3cat = 1              then glucose3cat = 1; 
  if HbA1c3cat = 0 | FPG3cat = 0              then glucose3cat = 2; 
