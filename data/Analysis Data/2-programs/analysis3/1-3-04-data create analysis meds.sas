proc sort data = jhsV3.medcodes; by subjid; run;

*MEDCODES;
data include1; *MEDCODES;
  set jhsV3.medcodes(keep = subjid tccode);
  by subjid;
...
*MSRC;
data include2; *MSRC;
  set jhsV3.msrc(keep = subjid msrc1 msrc2 msrc29a); 
  by subjid;

  *Create renamed duplicates for formulas;
  BPmedsSelfRep = msrc29a;

  *Create renamed duplicates for formulas;
  broughtMeds = "NotAllMeds";
    if msrc2 = 1      then broughtMeds = "AllMeds   ";
    if missing(msrc2) then broughtMeds = "          ";

  whyNOmeds = "Other    ";
    if msrc1 = 2      then whyNOmeds = "NoneTaken";
    if missing(msrc1) then whyNOmeds = "         ";
  run;
...
data include; *Create Variables;
  ...
  *Variable: antiArythMeds;
  ...
  format antiArythMeds ynfmt.;  run;
