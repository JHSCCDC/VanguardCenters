...
proc gplot data = validation;
symbol1  pointlabel=("#outlier_text_ff1" c=red h=1 );
...
data validation;
   ...
   if FPG>200 then do;
  		outlier = 1; *indicator for outlier status;
		outlier_text_fh1= cats("Outlier: ", subjid);
  	end;
  run; *proc print data=validation(where=(outlier=1)); run;
...
title3 "HbA1c Classified Normal";
  proc gplot data = validation(where = (HbA1c3cat = 0));
    symbol1  pointlabel=("#outlier_text_fh1" c=red h=1 );
  plot HbA1c*FPG = FPG3cat / vaxis=axis1 vref  = 5.7 href = 60 100 126;
    run; quit;
...
data validation;
   ...
   if FPG<40 then do;
  		outlier = 1; *indicator for outlier status;
		outlier_text_hf1= cats("Outlier: ", subjid);
  	end;
  run; *proc print data=validation(where=(outlier=1)); run;
...
title3 "FPG Classified Normal";
  proc gplot data = validation(where = (FPG3cat = 0));
    symbol1  pointlabel=("#outlier_text_hf1" c=red h=1 );
...
title2 "FPG vs HbA1c by Diabetic Classification";
  proc gplot data = validation;
  symbol1  pointlabel=none;
  plot FPG*HbA1c=diabetes / vaxis=axis1 haxis = 3 to 18 by 1 href  = 5.7 6.5 vaxis = 60 to 52 by 20 vref  = 60 100 126;
    run; quit;
...
data validation;
   ...
   if HbA1c>14 then do;
  		outlier = 1; *indicator for outlier status;
		outlier_text_fh_nomed= cats("Outlier: ", subjid);
  	end;
  run; *proc print data=validation(where=(outlier=1)); run;

title3 "Not on Diabetic Meds";
  proc gplot data = validation(where = (DMMeds = 0));
 symbol1  pointlabel=("#outlier_text_fh_nomed" c=red h=1 );
