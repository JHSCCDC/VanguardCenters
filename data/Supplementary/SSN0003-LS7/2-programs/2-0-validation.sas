...
%macro simple(visit =); *Macro for simple statistics;
  ...
  %if (&cont = ) %then
    %do;
      *Print first 5 obs of dataset;
      title2 "Visit &visit - Simple data listing (first 5 obs)";
      proc print data = validation(obs = 5) noobs;
        var subjid &catvar;
        run;

      *Summary statistics for cat vars;
      title2 "Visit &visit - Univariate summary stats: categorical";
  ...
  %if (&cat = ) %then
    %do;
      ...
      *Summary statistics for cont vars;
      title2 "Visit &visit - Univariate summary stats: continuous";
